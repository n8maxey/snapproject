...
# Create the regression table with exponentiated ORs and 95% CIs
table3 <- tbl_regression(
  ...
  modify_table_styling(
    columns = label,
    footnote = "Polynomial contrasts applied to ordinal variables: L = Linear, Q = Quadratic, C = Cubic, ^4 = Quartic trends"
  )
...
# Survey-weighted 80th percentile risk threshold
...
unweighted_80 <- quantile(prob_insecure, 0.80)

cat(sprintf("  Survey-weighted 80th percentile: %.4f\n", risk_threshold))
cat(sprintf("  Unweighted 80th percentile: %.4f\n", unweighted_80))
cat(sprintf("  Difference: %.4f\n\n", risk_threshold - unweighted_80))
...
results_summary <- tibble(
  Metric = c(
    ...
    "Risk threshold (weighted 80th %ile)",
...
for (k in 1:4) {
  ...
  # Bin into deciles (10 equal-sized groups)
  pred_deciles <- ntile(pred_k, 10)
  decile_labels <- paste0("D", pred_deciles)
  ...
  # Summarize by decile (survey-weighted)
...
# Create decile groups (10 equal-sized groups)
calibration_risk <- calibration_risk %>%
  dplyr::arrange(predicted_risk) %>%
  dplyr::mutate(
    decile = ntile(predicted_risk, 10)
  )
...
decile_summary <- calibration_risk %>%
  ...
  dplyr::mutate(
    ...
    top20_color = ifelse(decile >= 9, "Top 20%", "Lower 80%")
  )
...
fig3 <- ggplot(decile_summary, aes(x = pred_risk, y = obs_risk, color = top20_color, size = weight_sum)) +
  ...
  annotate("text", x = risk_threshold + 0.01, y = 0.05, hjust = 0, size = 3, color = "orange",
           label = paste0("80th %ile = ", round(risk_threshold, 3))) +
  scale_color_manual(values = c("Top 20%" = "#d62728", "Lower 80%" = "steelblue")) +
  scale_size_continuous(name = "Weighted N", breaks = c(5e4, 1e5, 1.5e5)) +
  geom_text(aes(label = decile_label), nudge_x = 0.02, nudge_y = 0.02, size = 3) +
  labs(
    title = "Figure 3: Grouped Risk Calibration Plot",
    subtitle = "Decile-Binned (10 Groups) Survey-Weighted Predictions vs Observed Hardship",
    x = "Mean Predicted P(Food Hardship)",
    y = "Observed Proportion with Hardship",
    color = "Risk Group",
    caption = paste0("Vertical line: 80th percentile threshold (", round(risk_threshold, 3), 
...
cat(sprintf("1. Risk Threshold (Weighted 80th Percentile): %.4f\n", risk_threshold))
cat(sprintf("   → Households with P(food insecurity) ≥ %.4f classified as HIGH-RISK\n", risk_threshold))
cat(sprintf("   → Represents top 20%% of population by predicted risk\n\n"))
